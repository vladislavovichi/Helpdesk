...
_SENTIMENT_MODEL_ID = "sentiment-heuristic-v1"
_TEXT_SIGNAL_RE = re.compile(r"[^\W\d_]", re.UNICODE)
_REPEATED_PUNCTUATION_RE = re.compile(r"[!?]{3,}")
_STRONG_ESCALATION_PHRASES = (
    "жалоба",
    "претенз",
    "в суд",
    "роспотребнадзор",
...
class AIApplicationService:
    ...
    async def analyze_ticket_sentiment(
        ...
        return _analyze_ticket_sentiment(command)
...
def _analyze_ticket_sentiment(
    command: AnalyzeTicketSentimentCommand,
) -> AnalyzedTicketSentimentResult:
    normalized = _normalize_sentiment_text(command.text)
    if normalized is None:
        return AnalyzedTicketSentimentResult(
            available=False,
            model_id=_SENTIMENT_MODEL_ID,
        )

    strong_hits = _count_phrase_hits(normalized, _STRONG_ESCALATION_PHRASES)
    frustration_hits = _count_phrase_hits(normalized, _FRUSTRATION_PHRASES)
    profanity_hits = _count_phrase_hits(normalized, _PROFANITY_MARKERS)
    punctuation_signal = 1 if _REPEATED_PUNCTUATION_RE.search(command.text or "") else 0
...
def _normalize_sentiment_text(text: str | None) -> str | None:
    if text is None:
        return None
    normalized = " ".join(text.lower().replace("ё", "е").split())
    if not normalized:
        return None
    if not _TEXT_SIGNAL_RE.search(normalized):
        return normalized
    return normalized
...
def _has_excessive_uppercase(text: str | None) -> bool:
    if text is None:
        return False
...
def _recent_unanswered_client_streak(messages: Sequence[AIContextMessage]) -> int:
    streak = 0
